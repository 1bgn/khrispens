...
pub async fn add_file_to_session(
    ...
    if let Some(mut bundle) = guard.get_mut(&s.session_number)
    {
        ...
        if let Some(folder) =  bundle.included_folders.get_mut(&s.root_folder_id){
            ...
            let _ = app_state.move_of(s.session_number).send(Message::Text(serde_json::to_string(&WebsocketEventObject { folder:s.root_folder_id,websocket_event_type: WebsocketEvent::FileEvent, data: file.clone() }).unwrap()));
